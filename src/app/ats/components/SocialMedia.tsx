...
import { QRCodeInvalititate } from '@/app/cv/components/SocialMedia.Invalitita.style';
...
const SocialMedia: React.FC<SocialMediaProps> = ({ hidden = false, invalitita = false }) => {
    ...
    return (
        <SocialMediaContainer>
            ...
            <SocialMediaGrid>
                ...
                {invalitita && !hidden && <QRCodeInvalititate />}

                {hidden &&
                    data.map((sm, index) => (<SocialHidden href={sm.url}
                        key={index}  >{sm.url}</SocialHidden>))}
                {invalitita && hidden && ''}
                {invalitita && hidden && <SocialHidden href='https://servizi2.inps.it/servizi/VerificaQRCD/verifica?verificaQR=AQIDBAUGBwgJCgsMDQ4PEKVqCzvdDQbm8QI_dqfVuWDfM8wFD3tp5ZfssVgCvUFDw6FSVmCEWwiPEgVWUbszng%3d%3d'>https://servizi2.inps.it/servizi/VerificaQRCD/verifica?verificaQR=AQIDBAUGBwgJCgsMDQ4PEKVqCzvdDQbm8QI_dqfVuWDfM8wFD3tp5ZfssVgCvUFDw6FSVmCEWwiPEgVWUbszng%3d%3d</SocialHidden>}
            </SocialMediaGrid>
        </SocialMediaContainer>
    )
}
